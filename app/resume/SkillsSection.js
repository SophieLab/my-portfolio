    ...
    <div className="grid grid-cols-1 gap-6">
      ...
      <Card className="w-full group hover:bg-white transition duration-300 p-6 rounded-lg shadow-lg">
        ...
        <div className="flex items-center justify-center gap-6 text-lg">
          {resumeData.cv.skills.skillCategories[0].items.map((skill) => (
            <div key={skill.name} className="flex items-center gap-4">
              <span className="text-indigo-900">
                <img
                  src={skill.icon.props.src}
                  alt={skill.icon.props.alt}
                  className="w-14 h-14 object-contain" // Taille augmentée ici
                />
              </span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {resumeData.cv.skills.skillCategories.slice(1).map((category) => (
          <Card key={category.category} className="group hover:bg-white transition duration-300 p-6 rounded-lg shadow-lg">
            <h4 className="text-2xl font-semibold mb-4 text-black group-hover:text-indigo-600 text-center">
              {category.category}
            </h4>
            <div className="flex items-center justify-center gap-6 text-lg">
